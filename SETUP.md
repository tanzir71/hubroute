...
2. Upload these files to the target folder, usually `public_html/`: `hubroute.php`, `index.html`, `README.md`, `SETUP.md`, `SECURITY.md`, `CHANGELOG.md`, `.env.example`.
3. Copy `.env.example` to `.env`, edit values if needed, and keep `.env` out of Git.
4. Create `data/` next to `hubroute.php`. Prefer permissions `750` or `770`; use `775` only if your host requires it.
5. Visit `https://yourdomain.com/hubroute.php`. First run creates `data/hubroute.sqlite`, `data/php-error.log`, and denial files inside `data/`.
