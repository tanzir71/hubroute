...
export function money(cents){
  return 'Tk ' + (Number(cents || 0) / 100).toLocaleString('en-BD', { maximumFractionDigits: 0 });
}

export function fmtDate(iso){ return new Date(iso).toLocaleString([], {month:'short', day:'numeric', hour:'2-digit', minute:'2-digit'}); }
...
export function pill(status){ return `<span class="pill ${statusClass(status)}">${esc(String(status).replaceAll('_',' '))}</span>`; }
