...
function tableCell(string $label, string $html, string $class = ''): string
{
    ...
    return '<td data-label="' . e($label) . '"' . $classAttr . '>' . $html . '</td>';
}

function monoValue(string $value): string
{
...
function renderLayout(string $title, ?array $user, string $content, array $meta = []): void
{
    ...
    echo '<style>
    :root{--bg:#f4f5f7;--panel:#ffffff;--panel-2:#f8f9fb;--text:#141821;--muted:#5c6470;--line:#e5e8ee;--line-strong:#d3d8e0;--ink:#0b0d12;--brand:#c2410c;--brand-ink:#9a3412;--brand-soft:#fff7ed;--ok:#0f7a4d;--warn:#9a5b00;--danger:#c0362c;--mono:ui-monospace,"SF Mono","JetBrains Mono","Roboto Mono",Menlo,Consolas,monospace;--shadow:0 16px 40px rgba(11,13,18,.14);--band:var(--panel-2);--soft:var(--brand-soft);--blue:var(--brand);--green:var(--ok);--amber:var(--warn);--red:var(--danger);}
    *{box-sizing:border-box}body{margin:0;font:14px/1.45 Inter,ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;color:var(--text);background:var(--bg)}
    ...
    .table .btn{min-height:28px;padding:5px 9px;font-size:12.5px}
...
function pageAgentRun(PDO $pdo, array $u): void
{
    ...
    if (count($rows) === 0) {
        ...
        foreach ($rows as $p) {
            ...
            $content .= tableCell('Dropoff', e((string)$p['dropoff_address']), 'muted');
            $content .= tableCell('COD', monoValue(formatMoney((int)$p['amount_cents'])));
            $content .= tableCell('Customer', e((string)$p['customer_name']), 'muted');
            $content .= '</tr>';
        }
        $content .= '</tbody></table>';
    }
...
function pageSettlements(PDO $pdo, array $u): void
{
    ...
    foreach ($ps as $p) {
        ...
        $content .= tableCell('Tracking', '<a class="mono" href="?r=parcel&id=' . (int)$p['id'] . '">' . e((string)$p['tracking_code']) . '</a>');
        $content .= tableCell('Customer', e((string)$p['customer_name']), 'muted');
        $content .= tableCell('Amount', monoValue(formatMoney((int)$p['amount_cents'])));
        $content .= tableCell('Status', statusPill((string)$p['status']));
        $content .= tableCell('Settled', ((int)$p['cod_settled'] === 1 ? uiPill('yes', 'bg-green') : uiPill('no', 'bg-amber')));
        $content .= '<td data-label="Action">';
